                ...
                @foreach (App\Models\Cart::where('user_id', Auth::guard('customer')->user()->id)->get() as $cart)
                    ...
                    <li class="">
                        <a href="#" class="sidecart_pro_img">
                            <img src="{{ asset('public/' . api_asset($cart->product->thumbnail_image)) }}" alt="product">
                        </a>
                        <div class="ec-pro-content">
                            <a href="{{ route('search', $cart->product->slug) }}?type=product"
                          ...
                          {{--  <div class="row">
                                <button type="button" class="btn btn-danger btn-number"
                                    onclick="update_qty('minus',{{ $cart->product_id }},{{ $product_prices['min_qty'] > 0 ? $product_prices['min_qty'] : 'null' }},'ajax')"
                                    style="width: auto;">
                                    <span class="ecicon eci-minus"></span>
                                </button>
                                <input type="number" id="quantity" name="product_qty"
                                    class="form-control text-center qty_value_{{ $cart->product_id }}"
                                    value="{{ $cart->quantity }}" min="{{ $cart->product->retailer_min_qty }}"
                                    max="{{ $cart->product->retailer_max_qty }}"
                                    style="width:60px; padding: 0 10px; height: auto;">
                                <button type="button" class="btn btn-danger btn-number btn-number"
                                    onclick="update_qty('plus',{{ $cart->product_id }},{{ $product_prices['max_qty'] > 0 ? $product_prices['max_qty'] : 'null' }},'ajax')"
                                    style="width: auto;">
