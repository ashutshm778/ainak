                        ...
                        <div class="col-md-12 col-sm-12">
                            <ul class="ec-breadcrumb-list">
                                ...
                                <li class="ec-breadcrumb-item active">
                                    {{ App\Models\Admin\Category::where('id', $data->category_id[0])->first()->name }}</li>
                                <li class="ec-breadcrumb-item active">
                                    {{ App\Models\Admin\SubCategory::where('id', $data->subcategory_id[0])->first()->name }}
                                </li>
                                    ...
                                    <div class="row">
                                        <div class="ec-ratting-content">
                                            <div class="ec-ratting-form">
                                                @php
                                                    $reviews = App\Models\Review::where('product_id', $data->id)
                                                        ->where('status', 1)
                                                        ->get();
                                                @endphp
                                                            ...
                                                            <div class="col-md-6">
                                                                <div class="ec-ratting-input">
                                                                    <input placeholder="Name" name="name"
                                                                        type="text"
                                                                        value="{{ optional(Auth::guard('customer')->user())->first_name }}"
                                                                        required />
                                                                </div>
                                                            </div>
                                                            <div class="col-md-6">
                                                                <div class="ec-ratting-input">
                                                                    <input placeholder="Email*" name="email"
                                                                        type="email"
                                                                        value="{{ optional(Auth::guard('customer')->user())->email }}">
                                                                </div>
                                                            </div>
                                                        </div>
                                                ...
                                                @else
                                                    <a type="button" class="btn btn-primary"
                                                        href="{{ route('user.login') }}?from={{ url()->full() }}">
                                                        Login For Review</a>
                                                @endif
                                            </div>
                                        </div>
    ...
    <section class="section ec-trend-product section-space-p">
        <div class="container">
            ...
            <div class="ec-trend-slider">
                @foreach (App\Models\Admin\Product::wherejsonContains('category_id',$data->category_id[0])->get() as $product)
                @php
                $productData = $product;
                $productprice = homePrice($product->id);
                @endphp
